...
rep_dataset <- rep_dataset %>%
  ...
  group_by(irsad,sex,inst_group) %>%
  summarise(n = sum(n))

#adding column for proportion
rep_dataset <- rep_dataset %>%
  mutate(pct=(n)/sum(n))
...
rep_dataset %>%
  ggplot(aes(x = sex,
             y = pct)) +
  geom_bar(aes(fill = inst_group),stat = "identity") + 
  facet_grid(cols = vars(irsad)) + 
  labs(x = "", y="Proportion of group", title = "VET/University split for 20-24 year-olds, by IRSAD SES decile")+
